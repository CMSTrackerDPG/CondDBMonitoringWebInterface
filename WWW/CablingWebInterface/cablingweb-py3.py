        ...
        if options.det6=="dc":
            ...
            for p in li5:
                if p in  DictionaryCab.CablingInfoDict:
                    ...
                    archim.write(p+" "+" 255 153 255"+"\n")
        archim.close()
        detstr=", ".join(options.lismod)
        os.system(('print_TrackerMap %s  "TrackerMap DetID %s" %s 2400 False False 999 -999 True')%(options.det4,detstr,options.det5))
        ...
        if options.lisfetrc1=="FedId":
            ...
            for x in lisf1:
                if x not in lisf1n:
                    ...
                    txtc1.write(x+"\n")
            txtc1.close()
            fedstr=", ".join(options.lisfetrc2)
            os.system(('print_TrackerMap %s  "TrackerMap FED %s" %s 2400 False False 999 -999 True')%(options.fe3, fedstr,options.fe4))
...
#FF        StripDetIDAliasDict=pickle.load(open(os.getenv("CMSSW_RELEASE_BASE")+"/src/CalibTracker/SiStripDCS/data/StripDetIDAlias.pkl"))
        StripDetIDAliasDict=pickle.load(open(os.getenv("CMSSW_BASE") + "/src/CalibTracker/SiStripDCS/data/StripDetIDAlias.pkl"))
        MyAlias=AliasFun(cablingfile,options,StripDetIDAliasDict)

        MyCabList=DetIdCabL(cablingfile)

        fileHV1=os.getenv("CMSSW_BASE") + '/src/CalibTracker/SiStripDCS/data/StripPSUDetIDMap_BeforeJan132010.dat'
...
#FF        fileHV1='/afs/cern.ch/cms/slc6_amd64_gcc481/cms/cmssw/CMSSW_7_0_4/src/CalibTracker/SiStripDCS/data/StripPSUDetIDMap_BeforeJan132010.dat' 
    ...
    if options.fileurl:
        ...
        ifile=open(os.getenv("CMSSW_BASE") + "/src/CalibTracker/SiStripDCS/data/StripDetIDAlias.pkl", "rb")
    ## for the alias
#FF        StripDetIDAliasDict=pickle.load(open(os.getenv("CMSSW_RELEASE_BASE")+"/src/CalibTracker/SiStripDCS/data/StripDetIDAlias.pkl"))
